...
ROUND(AVG(SALARY))"평균", SUM(SALARY)"급여합계", COUNT(*)"인원수"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1),'1','남','3','남','2','여','4','여')
ORDER BY COUNT(EMP_NAME) DESC;

--4. 부서내 성별 인원수를 구하세요.
SELECT DEPT_CODE"부서코드", COUNT(DECODE(SUBSTR(EMP_NO,8,1),'1','남'))"남자", COUNT(DECODE(SUBSTR(EMP_NO,8,1),'2','여'))"여자"
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE;
--쌤꺼
...
--부서명, 급여평균을 출력하세요.(GROUP BY로 산출한 결과에 대해 조건을 걸어야한다.)
SELECT DEPT_CODE"부서명", FLOOR(AVG(SALARY))"평균"
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
SELECT MANAGER_ID"매니져아이디", COUNT(*)"사원수"
FROM EMPLOYEE
GROUP BY MANAGER_ID
